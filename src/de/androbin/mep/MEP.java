...
public final class MEP
{
	...
	private static void addToken( final MathContext context, final List<Token> tokens, final List<Term> terms, final StringBuilder builder )
	{
	...
	public static Term parseTerm( final MathContext context, final String expression )
	{
		...
		if ( testForBrackets( expression ) )
		{
			...
			do
			{
				...
				if ( e.length() == expression.length() && !tokens.isEmpty() && tokens.get( 0 ) == Token.MINUS )
				{
					tokens.remove( 0 );
	...
	private static void parseTokens( final MathContext context, final String expression, final List<Token> tokens, final List<Term> terms )
	{
